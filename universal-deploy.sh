...
# Более мягкая проверка домена (разрешаем дефисы, точки, различные TLD)
if ! [[ $DOMAIN =~ ^[a-zA-Z0-9]([a-zA-Z0-9\.-]*[a-zA-Z0-9])?$ ]] || [[ ${#DOMAIN} -lt 4 ]] || [[ ${#DOMAIN} -gt 253 ]]; then
    echo "⚠️  Предупреждение: Домен '$DOMAIN' может иметь нестандартный формат"
